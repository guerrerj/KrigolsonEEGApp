...
export class BciComponent implements OnInit, OnDestroy,  AfterViewChecked {
  ...
  constructor(	private incomingData: DataService, private messagesService: MessagesService,
               public auth: AuthService, private afStorage: AngularFireStorage,
  ...
  addBciData(spectraData: any): void {
    ...
    for (let i = 0; i < this.settings.nChannels; i++) {
      ...
      if ( this.selectedElectrodeIdx === i ){
        ...
        const bar = this.AvgfreqOfChannel[this.selectedElectrodeIdx][j];
        const avgOfTenBar = this.avgOfTenAvgList[j];
    ...
    stopRecord(modalID: string): void {
      const csvData = this.selectedElectrode + ',' + this.recordedData.join(',') + '\n';
